"""FastMCP server — Windows desktop control via MCP.

v2.1.1 — Blocker fixes:
  BL#1: add PILImage import (NameError)
  ...
  PS: single process-wide _input_lock
"""
...
import json
from PIL import Image as PILImage  # BL#1: was missing
...
# ─── Capability Profiles (fail-closed) ─────────────────────────────────────
...
class PathPolicy:
    ...
    def resolve(self, value: str, destructive: bool = False) -> Path:
        ...
        # For destructive ops, forbid deleting the root itself
        if destructive and any(path == root for root in self.roots):
            raise PermissionError(
                f"Deleting an allowed root directory is forbidden: {path}"
            )
        return path

_path_policy = PathPolicy()
path_policy = _path_policy  # public export for remote module
...
class Frame:
    ...
    def to_screen(self, x: int, y: int) -> Tuple[int, int]:
        # PS: bounds check
        if not 0 <= x < self.image_width:
            raise ValueError(
                f"x={x} is outside frame bounds [0, {self.image_width})"
            )
        if not 0 <= y < self.image_height:
            raise ValueError(
                f"y={y} is outside frame bounds [0, {self.image_height})"
            )
    ...
    def validate(self) -> bool:
        if self.desktop_hash != _desktop_geometry_hash():
            return False
...
def _make_frame(screenshot: Dict[str, Any], image: PILImage.Image,
    ...
    frame = Frame(
        frame_id=frame_id, capture_kind=capture_kind, desktop_hash=desktop_hash,
        window_id=window_id,
        left=left, top=top,
...
def _screenshot_to_result(
    ...
    image_content = Image(data=buf.getvalue(), format="png").to_image_content()
    ...
    return ToolResult(
        content=[
            ...
            TextContent(
                type="text",
                text=json.dumps({"frame": frame_data}),
            ),
        ],
        structured_content={"frame": frame_data},
        meta=frame_data,
    )
...
def _resolve_coords(
    ...
    if frame is None:
        raise RuntimeError(f"Unknown frame_id '{frame_id}'. Re-screenshot.")
    if not frame.validate():
        _frame_registry.pop(frame_id, None)
        raise RuntimeError(f"Frame '{frame_id}' expired. Re-screenshot.")
    return frame.to_screen(x, y)


# ─── FastMCP Server Factory — BL#2/BL#3 fixed ─────────────────────────────

# PS: single process-wide input lock (shared across remote + local)
_PROCESS_INPUT_LOCK = asyncio.Lock()
...
def get_input_lock() -> asyncio.Lock:
    """Return the single process-wide input lock (for remote sharing)."""
    return _PROCESS_INPUT_LOCK
...
def create_server() -> FastMCP:
    ...
    # BL#2: instructions= not description=
    mcp = FastMCP(
        "Computer Use Windows",
        instructions="Windows desktop control via MCP — mouse, keyboard, UIA, files, system",
        version="2.1.4",
    )

    # ─── Helper: tool registration with annotations ────────────────────

    def _tool(**annotations):
        """Register tool with FastMCP 2.13.1 annotations format."""
        # BL#3: annotations dict, not kwargs
        return mcp.tool(annotations=annotations)

    # ═══════════ Observation ═══════════════════════════════════════════════

    @_tool(readOnlyHint=True, idempotentHint=True)
    ...
    async def computer_screenshot_window(window_id: int) -> ToolResult:
    ...
    async def computer_mouse_down(
        ...
        # PS: None,None = don't move cursor
        if (x is None) != (y is None):
            return {"success": False, "error": "x and y must be provided together"}
    ...
    @_tool(destructiveHint=True)
    @require_capability("mouse_up")
    async def computer_release_input_state() -> Dict[str, Any]:
        """Emergency: release all modifier keys (Shift/Ctrl/Alt/Win) + mouse buttons."""
        async with _PROCESS_INPUT_LOCK:
            return await a.release_input_state()

    # ═══════════ Clipboard ════════════════════════════════════════════════

    @_tool(readOnlyHint=True)
    ...
    @_tool(destructiveHint=True, idempotentHint=True)
    ...
    async def computer_get_file_permissions(path: str) -> Dict[str, Any]:
        ...
        return await a.get_file_permissions(str(safe))
    ...
    async def computer_find_element(role: Optional[str] = None, title: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def computer_find_ui_elements(
        control_type: Optional[str] = None, name: Optional[str] = None,
        automation_id: Optional[str] = None, class_name: Optional[str] = None,
    ) -> Dict[str, Any]:
    ...
    async def computer_delete_file(path: str) -> Dict[str, Any]:
        safe = _path_policy.resolve(path, destructive=True)
...
def enable_dpi_awareness() -> None:
    """DEPRECATED: use handlers.initialize_windows_runtime() instead."""
    from ..handlers.windows import initialize_windows_runtime
